...
int FPacketManage::ReadLastPacket(){
    ...
    // 开始读取数据
	readPacketSize = SerialClass->ReadDataUtil(PacketBuff, START_CODE_2, BytesNeeded);
    // 如果读取的数据大小不等于期望的数据包大小，则数据出错，丢弃读取到的数据，不进行任何处理， 返回-1
    if(readPacketSize != PACKET_SIZE - 1){
        UE_LOG(PacketManage, Warning, TEXT("数据包出错！ readPacketSize: %d , BytesNeeded : %d, START_CODE_2 : %d"), readPacketSize, BytesNeeded, START_CODE_2);
