...
//DEFINE_LOG_CATEGORY(FPacketManage);
...
bool FPacketManage::Connect(float DeltaTime){
    ...
    if(ConnectWaitedTime > ExpectConnectWaitTime){
        bool result = SerialClass->Open(4, 115200);
        if( result == false){
...
int FPacketManage::UpdatePlayerPose(FQuat * PlayerBonePoses, int BoneNums){
...
int FPacketManage::ReadLastPacket(){
    int ReadyBytesSize =  SerialClass->GetReadySize();
    // 将数据流定向到数据包开始，不包括START_CODE
    SerialClass->ReadDataUtil(PacketBuff, START_CODE, ReadyBytesSize);
    ...
    // 开始读取数据
    int readPacketSize = SerialClass->ReadDataUtil(PacketBuff, END_CODE, bytesNeeded);
...
int FPacketManage::Packet2Quat(FQuat * PlayerBonePoses, int BoneNums){
...
uint8 FPacketManage::dmpGetQuaternion(int16 *data, const uint8* packet) {
